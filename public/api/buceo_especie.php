...
if(isset($_GET['function'])){
    switch ($_GET['function']) {
        case "getEspeciesByIdBuceo":
            ...
            break;
        case "getEspecieByIdBuceo":
            echo getEspeciesByIdBuceo($_GET['buceoId']);
            break;
...
function getEspecieByIdBuceo($id){
    $especies = array();
    $link = connect_bd();
    $sql = "SELECT be.id_especie as id, e.nombre_comun, COUNT(be.id_especie) as count FROM buceo_especie as be LEFT JOIN especies as e ON be.id_especie = e.id WHERE be.id_buceo = (".$id.") GROUP BY id_especie";
     $result = mysqli_query($link,$sql);
    if( mysqli_num_rows($result) > 0 ){
        while ($row = mysqli_fetch_assoc($result)){
            array_push($especies, $row);
